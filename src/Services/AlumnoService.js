...
export class AlumnoService {
    ...
    getPeticionByUserId = async (id) => {
       try{
        ...
        let query=`SELECT Peticion.id, Peticion.Horario, Peticion.Estado, Profesor.nombre, Profesor.apellido from Peticion join Alumno on Peticion.idAlumno=Alumno.id where idAlumno = ?`
        const [result,fields] = await connection.execute(query,[id]);
        console.log(result);
        return result;
       }
